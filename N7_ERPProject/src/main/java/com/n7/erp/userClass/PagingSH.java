...
public class PagingSH {
	...
	public PagingSH(int maxNum, int pageNum, int listCount, int pageCount, String boardName, String keyword) {
		this.maxNum = maxNum;
	...
	private String makeHtml(int currentGroup, int totalPage, String boardName) {
		...
		if (start != 1) {
			sb.append("<a href='"+boardName+"?keyword="+keyword+"&pageNum=" + (end + 1) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
