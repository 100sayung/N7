...
public class Paging {
	...
	private String makeHtml(int currentGroup, int totalPage, String boardName) {
		...
		if (start != 1) {
			sb.append("<a style='text-decoration: none;' href='"+boardName+"?pageNum=" + (start -1) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a style='text-decoration: none;' href='"+boardName+"?pageNum=" + i + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a style='text-decoration: none;' href='"+boardName+"?pageNum=" + (end + 1) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
