...
<body>
	...
	매출
	<input onclick="getList('AS')" type='radio' name="sale" value='AS'>
	매입
	<input onclick="getList('AP')" type='radio' name="sale" value='AP'>
...
<script type="text/javascript">
	...
	$("#getshipment")
			.click(
					function() {
						$("#detailebutton")
								.html(
										"<button id='shipment' onclick='shipmentDetaile()' type='button'>등록하기</button> 출하<input class='changeship' type='radio' name='changeshipment' value='2'>입고<input class='changeship' type='radio' name='changeshipment' value='1'>");
						$("#plusorminus").html("");
						$("#testTable").html("");
						$("#insert").hide();
					});
	...
	$(document)
			.on(
					...
					function() {
						var num = "";
						$("input[name='changeshipment']:checked").each(
								function() {
									num = $(this).attr("value");
								});
						$
								.ajax({
									...
									success : function(data) {
										...
										for ( var i in data.sList) {
											...
											qty = Math
													.abs(Number(data.sList[i].ie_qty));
	...
	function shipmentDetaile() {
		...
		if (id.length != 0) {
			...
			window
					.open('/erp/rest/Account/shipmentDetaile?ARR=' + arr
							+ '&CNT=' + cnt, 'shipmentDetaile',
							'width=1400,height=700');
		}
	};
					...
					$.ajax({
						...
						success : function(data) {
							...
							if (data.sList.length == 1) {
								...
								window.open('/erp/Account/acApproval?check='
										+ check, 'acApproval',
										'width=1400,height=700');
							}
						},
